...
$review = $conn->query("SELECT r.*,concat(name,' ',username) as name FROM `rate_review` r inner join users u on r.user_id = u.id where r.package_id='{$id}' order by unix_timestamp(r.date_created) desc ");
    ...
    $(function(){
        $('#book').click(function(){
            ...
            if("<?php echo $_settings->userdata('id') ?>" > 0)
                {
                if("<?php echo isset($status) && $status == 1 ? 'selected' : '' ?>"==0)
                {   
                document.getElementById("book").disabled = true;
                alert_toast("Sorry,This Package Is Currently Unavailable","error");
                }
